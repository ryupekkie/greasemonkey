...
const checkType = type => {
  if (type.toLowerCase() === 'repair' || type.toLowerCase() === 'maintenance') {
    return true;
  }
  return false;
};
...
const trimRequest = (title, type, description, req, res, done) => {
  ...
  Joi.validate(request, createRequestSchema, err => {
    if (err) {
      return res
        .status(400)
        .json({ status: 'error', code: 400, message: err.details[0].message });
    }
...
const checkRequestId = (req, res, done) => {
  ...
  if (!requestId || !/^[0-9]{1,}$/.test(requestId) || Number.isNaN(requestId)) {
    return res
      .status(400)
      .json({ status: 'error', code: 400, message: 'requestId is not valid' });
  }
  return done();
};
...
const updateRequest = (req, res, done) => {
  if (req.body.title) {
    ...
    if (
      ...
      !/^[a-zA-Z0-9\s,]{1,}$/.test(req.body.title) ||
      req.body.title.length < 2
    ) {
      return res
        .status(400)
        .json({ status: 'error', code: 400, message: 'title is invalid' });
    }
  }
  if (req.body.type) {
  ...
  if (req.body.description) {
    ...
    if (
      ...
      !/^[a-zA-Z0-9,\.\s]{1,}$/.test(req.body.description)
    ) {
      return res
        .status(400)
        .json({
